//----------------------------------------------------------------------------
// A driver for modified Cholesky algorithms in the SE family.
//
// Reference:
...
int main(int argc, char *argv[]) {
    ...
    REAL delta = 0.0;
    bool gersh_pivot = true;  // false for no pivoting;
                              // true  for Gershgorin circle pivoting
    bool is_type1 = true;
    ...
    for (int i=1; i<argc; i++) {
        ...
        // pivoting method
        else if (!strcmp(argv[i], "-nopivot")) {
            if (gersh_pivot == false) {
                cout << "Error: algo (\"-nopivot=\") is specified more than once!" << endl;
                exit(1);
            }
            gersh_pivot = false;
        }
        ...
        else if (!strcmp(argv[i], "-no_special_last")) {
            if (special_last == false) {
                cout << "Error: no special treatment for the last 1-by-1 or 2-by-2 Schur complement (\"-no_special_last\") is specified more than once!" << endl;
                exit(1);
            }
            special_last = false;
        }
    ...
    if (algo == 0) {
        // SE90 algorithm
        info = mchol_se90(nrows, sa,
                         perm, modified);
    }
    else if (algo == 1) {
        // SE99 algorithm
        info = mchol_se99(nrows, sa,
                         perm, modified);
    }
    else if (algo == 2) {
        // SE-I algorithm
        info = mchol_se1(nrows, sa,
                         perm, modified);
    }
    else {
        // algo==-1, a generic member in the SE family
        info = mchol_se(nrows, sa,
                        perm, modified,
                        delta,
                        gersh_pivot,
...
void print_usage_and_exit(const char *cmd) {
    ...
    cout << "  Modified Cholesky algorithms in the SE family." << endl;
    ...
    cout << "  -P=MATRIX_FILE      save the permutation matrix P to MATRIX_FILE" << endl;
    cout << "  -E=MATRIX_FILE      save the diagonal modification matrix E to MATRIX_FILE" << endl;
    cout << "  -help               display this help and exit" << endl;
    cout << "  -se90               for the SE90 algorithm" << endl;
    cout << "  -se99               for the SE99 algorithm" << endl;
    cout << "  -se1                for the SE-I algorithm" << endl;
    cout << "                      if \"-se90\",  \"-se99\", or \"-se1\" is specified," << endl;
    cout << "                      then the following parameter specification is ignored" << endl;
    cout << "  -delta=VALUE        the modification tolerance delta" << endl;
    cout << "  -nopivot            for no pivoting in phase 2" << endl;
    cout << "                      (the default is Gershgorin circle pivoting in phase 2)" << endl;
    ...
    cout << "  -no_special_last    for not using the special treatment for the last" << endl;
    cout << "                      1-by-1 or 2-by-2 steps used in the SE99 algorithm" << endl;
    cout << "                      (the default invokes this special treatment)" << endl;
    cout << endl;
